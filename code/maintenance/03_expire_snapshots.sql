...
--       run.sh 경유 수동 실행: older_than = TODAY - 90d 를 계산해 아래 값을 교체한 뒤 실행.

CALL phm.system.expire_snapshots(
    table => 'bronze.engine_sensor_raw',
    older_than => TIMESTAMP '2026-02-08 00:00:00',  -- 갱신 기준: 2026-05-09 - 90일
    retain_last => 20
);

CALL phm.system.expire_snapshots(
    table => 'silver.engine_health',
    older_than => TIMESTAMP '2026-02-08 00:00:00',
    retain_last => 20
);
